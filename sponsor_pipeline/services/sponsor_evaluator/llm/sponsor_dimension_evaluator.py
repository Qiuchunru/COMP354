...
Concrete implementations:
  ClaudeSponsorDimensionEvaluator  : Anthropic Claude via tool use
  OpenAISponsorDimensionEvaluator  : OpenAI GPT via function calling
  GoogleSponsorDimensionEvaluator  : Google Gemini via function calling
>>>>>>> Stashed changes
"""
...
# ---------------------------------------------------------------------------
# OpenAISponsorDimensionEvaluator — calls the OpenAI API via function calling
# Same prompt text as the Claude implementation; only the SDK call differs
# ---------------------------------------------------------------------------

_OPENAI_CRITERION_TOOL = {
    "type": "function",
    "function": {
        "name": "record_criterion_score",
        "description": (
            "Record the evaluation score for a single sponsorship criterion. "
            "Call this function once with your final assessment."
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
_OPENAI_SUMMARY_TOOL = {
    "type": "function",
    "function": {
        "name": "record_evaluation_summary",
        "description": (
            "Record the holistic evaluation summary after all six criteria have been scored. "
            "Call this function once with the complete summary."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "recommended_contact_role": {
                    "type": "string",
                    "description": "Job title or role most likely to own the sponsorship decision",
                },
            },
            "required": [
                "motivations",
...
class OpenAISponsorDimensionEvaluator(SponsorDimensionEvaluator):
    """
    OpenAI-backed implementation of SponsorDimensionEvaluator.
    Uses function calling with tool_choice to guarantee structured output.
    """
    ...
    def evaluate_dimension(
        ...
        prompt = ClaudeSponsorDimensionEvaluator._build_dimension_prompt(
            criterion, company, evidence
        )
        response = self._client.chat.completions.create(
            model=self._model,
            messages=[
            ...
            tools=[_OPENAI_CRITERION_TOOL],
            tool_choice={"type": "function", "function": {"name": "record_criterion_score"}},
        )
    ...
    def evaluate_summary(
        ...
        prompt = ClaudeSponsorDimensionEvaluator._build_summary_prompt(
            company, evidence, criterion_scores
        )
        response = self._client.chat.completions.create(
            model=self._model,
            ...
            tools=[_OPENAI_SUMMARY_TOOL],
            tool_choice={"type": "function", "function": {"name": "record_evaluation_summary"}},
        )
        data = json.loads(response.choices[0].message.tool_calls[0].function.arguments)
        return SponsorEvaluationSummary(
            motivations=[SponsorMotivation(m) for m in data["motivations"]],
...
# ---------------------------------------------------------------------------
# GoogleSponsorDimensionEvaluator — calls the Google Gemini API
# Uses the new google-genai SDK (google.genai), not the deprecated
# google.generativeai package.
# ---------------------------------------------------------------------------

_GOOGLE_CRITERION_SCHEMA = {
    "type": "object",
    "properties": {
        "score": {"type": "number"},
...
_GOOGLE_SUMMARY_SCHEMA = {
    ...
    "properties": {
        ...
        "recommended_contact_role": {"type": "string"},
    },
    "required": [
        "motivations",
        "confidence",
...
class GoogleSponsorDimensionEvaluator(SponsorDimensionEvaluator):
    """
    Google Gemini-backed implementation of SponsorDimensionEvaluator.
    Uses response_mime_type + response_schema to guarantee structured JSON output.
    """

    def __init__(self, client, model: str = "gemini-1.5-flash") -> None:
        self._client = client
        self._model = model

    def _generate(self, system: str, user: str, schema: dict) -> dict:
        import json
        from google.genai import types
    ...
    def evaluate_dimension(
        ...
        data = self._generate(_DIMENSION_SYSTEM_PROMPT, prompt, _GOOGLE_CRITERION_SCHEMA)
        return CriterionScore(
            criterion_key=criterion.key,
            score=float(data["score"]),
            reasoning=data["reasoning"],
    ...
    def evaluate_summary(
        ...
        data = self._generate(_SUMMARY_SYSTEM_PROMPT, prompt, _GOOGLE_SUMMARY_SCHEMA)
        return SponsorEvaluationSummary(
            motivations=[SponsorMotivation(m) for m in data["motivations"]],
            confidence=Confidence(data["confidence"]),
            explanation=data["explanation"],
